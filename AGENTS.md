...
- **Thinking off in the prompt** (`volocal.suppressThinking`, default on). `llama_chat_apply_template` is **not Jinja**. Gemma 4 uses native `<|turn>`. **E2B/E4B must not get an empty thought primer** — that primer is 12B+ only and on the small models it *starts* CoT (“the user just said…”). Ban the `<|channel>` token. Skip a thought channel / leading “the user said…” / “I need to respond naturally…” planning sentences if they still appear. Gemma 3 R1 must **not** be prefilling `<think>` or `</think>`. Those tags are ordinary text. A closing tag at the start of the model turn makes the instructions and saved context look like the user's turns, and the tag is spoken. Strip `<think>` / `</think>` from the generated text and from saved assistant turns. Qwen 3 gets `/no_think`. Chat delimiters are stop sequences. Toggle is on the language-model screen. `SentenceBuffer` splits on `.!?` (or a ~360-char runaway cap). The **first** chunk may also be a comma clause of 4+ words, or the first 10 words, so TTS starts before a long sentence finishes. Later chunks stay on `.!?`. Newlines are spaces, not clause breaks — splitting on `\n` stole the next word’s first letter for TTS. Do **not** force-split on 40/64 character word crumbs — that pauses mid-phrase.
...
- PocketTTS emits **24 kHz** mono in 80 ms frames. Schedule each frame as it is generated; do not buffer the whole clause first. Parakeet stays paused while those frames are synthesized (ANE). Sustained loud mic energy (~0.28s above ~0.05 RMS) must still interrupt, because playback is already audible. Supertonic-3 emits **44.1 kHz** and must be resampled to 24 kHz before `SharedAudioEngine`. Do not add Dia/Orpheus. Inflect/NeuTTS are beta and not a guest fit (NeuTTS is stateful CoreML like Nano). Hugging Face “SOTA” TTS (Qwen3-TTS, Fish, CSM) is Python/CUDA — there is no iOS CoreML pack. `[laugh]` / `[gasp]` / `[sigh]` are `NonverbalSynth` cues. `[whisper]…[/whisper]` lowers the audio. Strip `<speak>` / `</speak>` if a model writes them; they are not a voice markup we render.
...
- Nemotron has **no EOU head**. `STTManager` treats ~900 ms of unchanged partial as a turn, or ~320 ms when the partial looks finished. `MicTurnDetector` ends a finished quiet-room turn after ~0.28 s, otherwise ~0.8 s.
